...
tf.app.flags.DEFINE_string('input_file', None, 'File with (path, label) tuples')
...
tf.app.flags.DEFINE_string('class_list', None, 'File with the labels')
...
def _is_png(filename):
    ...
    if '.png' in filename:
        ...
        file_type = subprocess.check_output(['file', '-b', '--mime-type', filename])
        return 'image/png' in str(file_type)
...
def _find_image_files(input_file, labels_list_path, dataset_dir):
    """
    Build a list of all images files and labels in the data set.
    Args:
        input_file: path to the file listing (path, label) tuples
        labels_list_path: path to the file with the class id -> class name mapping
    Returns:
        filenames: list of strings; each string is a path to an image file.
        texts: list of string tuples; each string is the tuple of classe, e.g. ('fear')
        labels: list of integer tuples; each tuple identifies the ground truth: (class_id)
    """
    lines = [line.strip() for line in open(input_file, 'r')]
    labels_list = [line.strip() for line in open(labels_list_path, 'r')]
    filenames = list()
    texts = list()
    labels = list()
    for line in lines:
        img, label_id = line.split()
        class_name = labels_list[int(label_id)]
        filenames.append(os.path.join(dataset_dir, class_name, img))
...
def _process_dataset(name, input_file, dataset_dir, num_shards, labels_list_path):
    """
    ...
    Args:
        ...
        input_file: path to the file listing (path, label) tuples
        num_shards: integer number of shards for this data set.
        labels_list_path: string, path to the labels file.
    """
    filenames, texts, labels = _find_image_files(input_file, labels_list_path, dataset_dir)
