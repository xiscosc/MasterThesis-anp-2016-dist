...
tf.app.flags.DEFINE_boolean('data_augmentation', False,
                            """Whether to randomly flip/crop the training images.""")
...
def inputs(dataset, batch_size=None, num_preprocess_threads=None):
    """
    Generate batches of MVSO images for evaluation.
    ...
    with tf.device('/cpu:0'):
        ...
        return batch_tensors  # (images, labels) or (images, labels[0], labels[1], labels[2])


def distorted_inputs(dataset, batch_size=None, num_preprocess_threads=None, data_augmentation=None):
    """
    ...
    Args:
      ...
      num_preprocess_threads: integer, total number of preprocessing threads but
        None defaults to FLAGS.num_preprocess_threads.
      data_augmentation: boolean; None defaults to FLAGS.data_augmentation.
    Returns:
      images: Images. 4D tensor of size [batch_size, FLAGS.image_size,
                                         FLAGS.image_size, 3].
...
def image_preprocessing(image_buffer, bbox, train, thread_id=0, data_augmentation=False):
...
def batch_inputs(dataset, batch_size, train, num_preprocess_threads=None, num_readers=1, data_augmentation=False):
    """
    ...
    with tf.name_scope('batch_processing'):
        ...
        for thread_id in range(num_preprocess_threads):
            ...
            image_buffer, label_index, bbox, _ = parse_example_proto(
                example_serialized)
            image = image_preprocessing(image_buffer, bbox, train, thread_id, data_augmentation)
            if FLAGS.multi_task:
                # label_index = tf.reshape(label_index, [1, 3])
                images_and_labels.append([image, label_index[0], label_index[1], label_index[2]])
            else:
                images_and_labels.append([image, label_index[0]])
